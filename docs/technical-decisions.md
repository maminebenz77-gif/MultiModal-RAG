...
**The public Langfuse Cloud is refused unconditionally, independent of `allow_external`.** `allow_external` answers "may this profile call any external service at all" — true on an ordinary company laptop, since it needs to reach the company's own external LLM gateway. That is a different question from "may this specific data (real user questions, answers, retrieved document content) be handed to a third party's cloud," which has to stay no even when the first answer is yes; conflating the two would mean a laptop set up to reach an approved company AI endpoint could just as easily leak confidential query content to `cloud.langfuse.com` the moment someone pasted in API keys. So `get_langfuse_client()` hard-blocks `cloud.langfuse.com` and its regional variants (`us.`/`eu.cloud.langfuse.com`) before the `allow_external` guard ever runs, and separately refuses to default a blank `LANGFUSE_HOST` to that same address the way the SDK itself would — tracing only activates once someone explicitly names a real (intended to be self-hosted) host. The existing `allow_external`/internal-host guard (§ privacy guard) still applies on top, for any *other* external address on an air-gapped profile. Every blocked case disables tracing with a logged warning rather than raising — tracing must never be a reason a real query fails, but silently going dark would leave a developer unable to tell why traces never show up.
