...
**The retrieval side is logged as an event, not a span.** `AgentChain.answer()`'s existing `on_tool_call(round_index, query, results)` hook (already used by `generation/demo.py` for console output) fires *after* a search completes, with no start time available — `log_search_event()` records it via `create_event()` (a point-in-time marker) rather than faking a zero-duration span.
