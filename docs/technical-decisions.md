...
- **Grounding/citation-format compliance is prompted, not enforced** (§11) — a model can still substitute an adjacent fact instead of refusing, or cite using its own trained-in format instead of the one requested; both were observed live with `gpt-4o-mini`, not just theorized. The refusal case was closed by naming it explicitly in the prompt; the citation-format case needed a code-level normalization fallback as well, since prompting alone didn't fully close it even after being strengthened.
