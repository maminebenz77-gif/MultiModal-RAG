...
**Tracing goes through the same `allow_external` privacy guard as every other network-facing provider** (§ privacy guard) — `langfuse_host=None` resolves to the SDK's own default (`https://cloud.langfuse.com`) *before* the guard check, not after, since checking the unresolved `None` would look like "no endpoint to check" and silently let a query's real content leak to the public cloud on an air-gapped profile. A blocked host disables tracing with a logged warning rather than raising — tracing must never be a reason a real query fails, but silently going dark would leave a developer unable to tell why traces never show up.
