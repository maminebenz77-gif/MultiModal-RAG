...
**Resumable via URL, with an explicit fallback boundary**: `conversation_id` lives in `st.query_params`, not just session state, so a refreshed tab reloads the transcript from the server instead of losing it. An unknown/stale id in the URL (a bookmark after a DB wipe) degrades to a fresh conversation rather than erroring — the same `try/except httpx.HTTPError` discipline the `/health` check already used, reused rather than reinvented. One asymmetry accepted deliberately: `retrieved_chunks` isn't part of what the server persists (only citations are — an explicit scope decision, see §13), so a reloaded turn has real citations but no "Retrieved chunks" expander, while a turn generated live later in the same session has both.
...
- **`retrieved_chunks` isn't persisted, only citations are** (§13) — a reloaded conversation can show what was cited but not the full retrieved-candidate set from the original turn. A deliberate scope decision, not an oversight; revisit if that history ever matters for evaluation.
- **No listing, deleting, or access control on conversations** (§13) — `conversation_id` is a bearer-style identifier with no ownership check, same trust model this app already has for `query_id`/`doc_id`. Fine for a single-user learning deployment, not for anything multi-tenant.
